...
import { Briefcase, CheckCircle2, Plus, Target, Trash2, TrendingUp } from 'lucide-react';
import { createCareerGoal, deleteCareerGoal, fetchCareerCatalog, fetchCareerGoals, fetchCareerRoadmap } from '../services/careerApi';
...
export const CareerRoadmapPage: React.FC = () => {
  ...
  const addGoal = async (role: string) => {
    setBusy(true); setError('');
    try { await createCareerGoal(role); await load(); }
    catch (e: any) { setError(e.message || 'Unable to create career goal'); }
    finally { setBusy(false); }
  };
  const removeGoal = async (id: string) => {
    setBusy(true);
    try { await deleteCareerGoal(id); setSelected(null); await load(); }
    catch (e: any) { setError(e.message || 'Unable to remove goal'); }
    finally { setBusy(false); }
  };
  ...
  return <div className="p-6 lg:p-8 max-w-7xl mx-auto space-y-8">
    <header>
      <div className="flex items-center gap-3"><Briefcase className="text-emerald-600" /><h1 className="text-3xl font-bold text-slate-900">AI Career & Skill Roadmap</h1></div>
      <p className="mt-2 text-slate-600">Turn your learning evidence into a practical path toward your target career.</p>
    </header>
    {error && <div className="rounded-xl bg-red-50 border border-red-200 p-4 text-red-700">{error}</div>}

    <section className="grid md:grid-cols-2 xl:grid-cols-4 gap-4">
      {catalog.map((career) => <button key={career.id} disabled={busy} onClick={() => addGoal(career.id)} className="text-left rounded-2xl border border-slate-200 bg-white p-5 hover:border-emerald-400 hover:shadow-sm transition">
        <Target className="text-emerald-600 mb-3" /><h2 className="font-semibold text-slate-900">{career.title}</h2><p className="mt-2 text-sm text-slate-500">{career.description}</p><span className="mt-4 inline-flex items-center gap-1 text-sm text-emerald-700"><Plus size={15}/> Set as goal</span>
      </button>)}
    </section>

    {goals.length > 0 && <section className="grid lg:grid-cols-[280px_1fr] gap-6">
      <div className="rounded-2xl border border-slate-200 bg-white p-4 space-y-2"><h3 className="font-semibold mb-3">My career goals</h3>{goals.map(g => <div key={g._id} className="flex items-center gap-2"><button onClick={async () => setSelected((await fetchCareerRoadmap(g._id)).data)} className="flex-1 text-left rounded-lg p-3 hover:bg-slate-50"><div className="font-medium">{catalog.find(c => c.id === g.targetRole)?.title || g.targetRole}</div><div className="text-xs text-slate-500">{g.status}</div></button><button title="Delete" onClick={() => removeGoal(g._id)} className="p-2 text-slate-400 hover:text-red-600"><Trash2 size={16}/></button></div>)}</div>
      {selected && <div className="space-y-6">
        <div className="rounded-2xl bg-slate-900 text-white p-6"><div className="text-sm text-emerald-300">Career readiness</div><div className="flex items-end gap-3 mt-1"><span className="text-5xl font-bold">{selected.readiness}%</span><TrendingUp className="mb-2 text-emerald-300"/></div><p className="mt-2 text-slate-300">Deterministic score calculated from your existing learning mastery.</p></div>
