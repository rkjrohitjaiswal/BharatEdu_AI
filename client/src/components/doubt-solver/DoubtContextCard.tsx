...
export const DoubtContextCard: React.FC<DoubtContextCardProps> = ({ context }) => {
  const masteryScore = context.masteryScore ?? 75;
  const riskLevel = context.riskLevel || 'LOW';
  const prereqs = context.prerequisiteConceptIds || [];
    ...
    <div className="p-4 rounded-2xl bg-slate-50 border border-slate-200 space-y-3">
      <div className="flex items-center justify-between">
        <span className="text-xs font-black uppercase text-indigo-600 tracking-wider flex items-center gap-1.5">
          <Brain className="w-4 h-4" /> Curriculum Context: {context.subject}
        </span>
      ...
      <div className="grid grid-cols-2 gap-2 text-xs">
        <div className="p-2 rounded-xl bg-white border border-slate-100 font-medium text-slate-700">
          ...
          <span className={`font-black ${riskLevel === 'HIGH' ? 'text-red-600' : 'text-emerald-600'}`}>
            {riskLevel}
          </span>
        </div>
        <div className="p-2 rounded-xl bg-white border border-slate-100 font-medium text-slate-700">
          <span className="text-[10px] font-bold text-slate-400 block uppercase">Prerequisites</span>
          <span className="font-bold text-slate-800">{prereqs.length} Prerequisites</span>
        </div>
      </div>
