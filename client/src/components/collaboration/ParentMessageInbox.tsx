...
export const ParentMessageInbox: React.FC<Props> = ({ messages, onAcknowledge }) => {
  return (
        ...
        <div key={m.messageId} className="p-4 bg-slate-900/60 border border-slate-800 rounded-2xl space-y-3 text-xs">
          ...
          {m.requiresAcknowledgement && (
            <div className="pt-2 border-t border-slate-800/60 flex items-center justify-between">
              <span className="text-amber-400 font-semibold text-[10px]">Acknowledge Teacher Guide</span>
              <button
              ...
              >
                <CheckCircle2 className="w-3.5 h-3.5" /> Confirm Home Support
              </button>
            </div>
          )}
        </div>
...
export default ParentMessageInbox;
