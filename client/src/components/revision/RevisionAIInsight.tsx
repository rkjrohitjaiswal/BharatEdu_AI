...
export const RevisionAIInsight: React.FC<RevisionAIInsightProps> = ({ summary }) => {
  if (!summary) return null;

  const { summaryMessage = 'Spaced-repetition plan generated from your actual practice & gap history.' } = summary;
    ...
    <div className="p-6 rounded-2xl bg-gradient-to-r from-slate-950 via-indigo-950 to-purple-950 text-white shadow-xl space-y-3">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h3 className="font-bold text-base text-white">AI Spaced-Repetition Strategy</h3>
        </div>
      ...
      <p className="text-xs text-slate-300">
        Review intervals automatically adapt based on your practice accuracy, gap severity, and exam countdowns.
      </p>
    </div>
  );
};
