...
export interface TeacherWeeklyPlanProps {
  plan: IWeeklyPlanDay[];
  aiGenerated?: boolean;
}

export const TeacherWeeklyPlan: React.FC<TeacherWeeklyPlanProps> = ({ plan, aiGenerated }) => {
    ...
    <div className="p-6 rounded-2xl bg-white border border-slate-200 shadow-sm space-y-4">
      <div className="flex items-center justify-between">
        <h3 className="font-bold text-slate-900 text-base flex items-center gap-2">
          ...
          <span>Weekly Teacher Action Plan</span>
        </h3>
        {aiGenerated && (
      ...
      <div className="grid grid-cols-1 sm:grid-cols-5 gap-3">
        {plan.map((dayPlan, idx) => (
          <div key={idx} className="p-4 rounded-xl border border-slate-200 bg-slate-50/60 flex flex-col justify-between space-y-2">
            <div>
              <span className="text-xs font-extrabold uppercase text-indigo-700 block mb-1">
                {dayPlan.day}
              </span>
              <p className="text-xs text-slate-700 leading-snug font-medium">{dayPlan.task}</p>
            </div>
            <div className="pt-2 border-t border-slate-200/60 text-[11px] text-slate-500 font-semibold truncate">
              Target: {dayPlan.focusTopic}
            </div>
          </div>
        ))}
      </div>
